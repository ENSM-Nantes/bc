...
irr::video::IImage *RadarCalculation::update(RadarScreen *aRadarScreen, const Terrain *terrain, const OwnShip *ownShip, const Buoys *buoys, const OtherShips *otherShips, irr::f32 weather, irr::f32 rain, irr::f32 tideHeight, irr::f32 deltaTime, uint64_t absoluteTime, GUIMain* aGuiMain)
{
  ...
  if (2*aGuiMain->getRadarPixelRadius() > aRadarScreen->getRadarImage()->getDimension().Width)
    {
      radarImage = aRadarScreen->getRadarImageLarge();
      radarImageOverlaid = aRadarScreen->getRadarImageOverlaidLarge();
    }
  else
    {
      radarImage = aRadarScreen->getRadarImage();
      radarImageOverlaid = aRadarScreen->getRadarImageOverlaid();
    }
  ...
  { IPROF("Set up");
    ...
    if (mIsMouseDown) {
      irr::f32 mouseCursorRangeXNm = (irr::f32)aGuiMain->getCursorPositionRadar().X/(irr::f32)radarRadiusPx*radarRangeNm.at(radarRangeIndex);//Nm
      irr::f32 mouseCursorRangeYNm = -1.0*(irr::f32)aGuiMain->getCursorPositionRadar().Y/(irr::f32)radarRadiusPx*radarRangeNm.at(radarRangeIndex);//Nm
    ...
    scan(terrain, ownShip, buoys, otherShips, weather, rain, tideHeight, deltaTime, absoluteTime); // scan into scanArray[row (angle)][column (step)], and with filtering and amplification into scanArrayAmplified[][]
  } { IPROF("Update ARPA");
...
void RadarCalculation::scan(const Terrain *terrain, const OwnShip *ownShip, const Buoys *buoys, const OtherShips *otherShips, irr::f32 weather, irr::f32 rain, irr::f32 tideHeight, irr::f32 deltaTime, uint64_t absoluteTime)
{
  ...
  for(irr::u32 i = 0; i<scansPerLoop;i++) { //Start of repeatable scan section
    ...
    for (irr::u32 currentStep = 1; currentStep<rangeResolution; currentStep++) { //Note that currentStep starts as 1, not 0. This is used in anti-rain clutter filter, which checks element at currentStep-1
      ...
      irr::f32 localX = absolutePosition.X + relX;
      irr::f32 localZ = absolutePosition.Z + relZ;
