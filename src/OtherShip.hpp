...
{
public:
  OtherShip (const std::string& aName, const std::string& aInternalName, const irr::core::vector3df& aLocation, std::vector<Leg> aLegsLoaded, irr::IrrlichtDevice* aDev);
  ~OtherShip();
...
private:
  ...
  std::vector<NavLight*> mNavLights;
  irr::f32 mHeight; //For radar
  irr::f32 mSolidHeight; //For radar
  irr::f32 mRcs;
  irr::f32 mRateOfTurn;
