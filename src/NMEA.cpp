...
void NMEA::updateNMEA(sTime& aTime)
{
  ...
  if (mOtherShips->getNumber() >= 0) { // only consider AIS if there are other ships
    ...
    for (auto ship : readyShips) {
      ...
      int fillBits;
      bool done;
      std::tie(data, fillBits) = AIS::generateClassAReport(mOtherShips, mOwnShip->getOffsetPos().Z, mOwnShip->getOffsetPos().X, mOwnShip->getOffsetZ(), mOwnShip->getOffsetX(), mTerrain, aTime.absoluteTime, ship);
